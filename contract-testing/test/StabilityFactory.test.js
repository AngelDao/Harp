...
contract("Factory and Proxy Tests", () => {
  ...
  describe("Stability Factory attributes on deployment", async () => {
    ...
    it("Sets correct lastSupply", async () => {
      const creator = await stabilityFactory.lastSupply();
      assert.equal(creator.toString(), tokens("2000000"));
    });
    it("Sets correct maxSupply", async () => {
      const creator = await stabilityFactory.maxSupply();
  ...
  describe("Farm actions", async () => {
    ...
    it("check max supply reached", async () => {
      ...
      const lastSupply = await stabilityFactory.lastSupply();

      let userBalance = await lusdToken.balanceOf(owner.address);
      let userBalance2 = await lusdToken.balanceOf(owner2.address);
      ...
      await ownerProxy.deposit(tokens("60"), { from: owner.address });
      await owner2Proxy.deposit(tokens("90"), { from: owner2.address });

      console.log("before");
      console.log("before");
      await ownerProxy.claim(false, { from: owner.address });
      console.log("after");
      await owner2Proxy.claim(false, { from: owner2.address });

      console.log("after");
      await ownerProxy.withdraw(tokens("60"), { from: owner.address });
      await owner2Proxy.withdraw(tokens("90"), { from: owner2.address });

      userBalance = await stringToken.balanceOf(owner.address);
      userBalance2 = await stringToken.balanceOf(owner2.address);

      const totalRewards = userBalance.add(userBalance2);

      const totalSupply = await stringToken.totalSupply();

      const stringStakingBalance = await stringToken.balanceOf(
        stringStaking.address
      );

      console.log("farmbalance", stringStakingBalance.toString());

      console.log(totalRewards.toString());
      console.log(totalSupply.toString());

      assert.equal(totalSupply.toString(), tokens("2005000"));
    });
  });
});
