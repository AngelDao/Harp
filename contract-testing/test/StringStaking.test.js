...
contract("STRING Staking Tests", () => {
  ...
  describe("Farm attributes on deployment", async () => {
    ...
    it("Sets correct lastSupply", async () => {
      const creator = await stringStaking.lastSupply();
      assert.equal(creator.toString(), tokens("2000000"));
    });
    it("Sets correct maxSupply", async () => {
      const creator = await stringStaking.maxSupply();
  ...
  describe("Farm actions", async () => {
    ...
    const handleDeposit = async (owner, owner2) => {
      await stringToken
        .connect(owner)
        .approve(stringStaking.address, tokens("1000"));
      await stringToken
        .connect(owner2)
        .approve(stringStaking.address, tokens("1000"));
      await gstringToken
    ...
    it("check max supply reached", async () => {
      ...
      const lastSupply = await stringStaking.lastSupply();

      let userBalance = await stringToken.balanceOf(owner.address);
      let userBalance2 = await stringToken.balanceOf(owner2.address);
      ...
      await stringStaking.connect(owner).deposit(tokens("60"));
      await stringStaking.connect(owner2).deposit(tokens("90"));

      await stringStaking.connect(owner).withdraw(0);
      await stringStaking.connect(owner2).withdraw(0);

      await stringStaking.connect(owner).withdraw(tokens("60"));
      await stringStaking.connect(owner2).withdraw(tokens("90"));
