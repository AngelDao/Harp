...
contract StabilityPool is LiquityBase, Ownable, CheckContract, IStabilityPool {
    ...
    function withdrawFromSP(uint256 _amount) external override {
        ...
        uint256 LUSDtoWithdraw =
            LiquityMath._min(_amount, compoundedLUSDDeposit);
        uint256 LUSDLoss = initialDeposit.sub(compoundedLUSDDeposit); // Needed only for event log
    ...
    function _triggerLQTYIssuance(ICommunityIssuance _communityIssuance)
        internal
    {
    ...
    function _computeLQTYPerUnitStaked(
         ...
         */
        uint256 LQTYNumerator =
            _LQTYIssuance.mul(DECIMAL_PRECISION).add(lastLQTYError);
        ...
        returns (uint256 ETHGainPerUnitStaked, uint256 LUSDLossPerUnitStaked)
    {
        ...
        uint256 LUSDLossNumerator =
            _debtToOffset.mul(DECIMAL_PRECISION).sub(lastLUSDLossError_Offset);
        uint256 ETHNumerator =
            _collToAdd.mul(DECIMAL_PRECISION).add(lastETHError_Offset);
    ...
    function _updateRewardSumAndProduct(
        ...
        uint256 newProductFactor =
            _LUSDLossPerUnitStaked >= DECIMAL_PRECISION
                ? 0
                : uint256(DECIMAL_PRECISION).sub(_LUSDLossPerUnitStaked);
    ...
    function _getETHGainFromSnapshots(
        ...
        uint256 firstPortion =
            epochToScaleToSum[epochSnapshot][scaleSnapshot].sub(S_Snapshot);
        uint256 secondPortion =
            epochToScaleToSum[epochSnapshot][scaleSnapshot.add(1)].div(
                DECIMAL_PRECISION
            );
    ...
    function _getLQTYGainFromSnapshots(
        ...
        uint256 secondPortion =
            epochToScaleToG[epochSnapshot][scaleSnapshot.add(1)].div(
                DECIMAL_PRECISION
            );
        ...
        returns (uint256)
    {
        ...
        uint256 compoundedDeposit =
            _getCompoundedStakeFromSnapshots(initialDeposit, snapshots);
        ...
        returns (uint256)
    {
        ...
        uint256 compoundedFrontEndStake =
            _getCompoundedStakeFromSnapshots(frontEndStake, snapshots);
    ...
    function _sendLUSDtoStabilityPool(address _address, uint256 _amount)
        internal
    {
    ...
    function _sendLUSDToDepositor(address _depositor, uint256 LUSDWithdrawal)
        internal
    {
        if (LUSDWithdrawal == 0) {
            return;
    ...
    function _setFrontEndTag(address _depositor, address _frontEndTag)
        internal
    {
        ...
        internal
    {
        ...
        uint256 currentS =
            epochToScaleToSum[currentEpochCached][currentScaleCached];
        uint256 currentG =
            epochToScaleToG[currentEpochCached][currentScaleCached];
    ...
    function _requireFrontEndIsRegisteredOrZero(address _address)
        internal
        view
    {
        require(
